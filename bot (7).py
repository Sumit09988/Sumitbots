...
# ============ ADDITIONAL FIREBASE (for devices) ============
def get_additional_firebase_apps():
    apps = []
    for i in range(2, 10):
        name = os.getenv(f"FB{i}_NAME")
        cred_json = os.getenv(f"FB{i}_CREDENTIALS")
        db_url = os.getenv(f"FB{i}_DB_URL")
        if name and cred_json and db_url:
            try:
                cred_dict = json.loads(cred_json)
                cred = credentials.Certificate(cred_dict)
                app = firebase_admin.initialize_app(cred, {'databaseURL': db_url}, name=f"fb_{i}")
                apps.append((app, db_url))
                print(f"✅ Additional Firebase '{name}' connected!")
            except Exception as e:
                print(f"❌ Error connecting Firebase '{name}': {e}")
    return apps
...
def get_total_devices_all_firebase():
    ...
    if not isinstance(db, FakeDB):
        try:
            device_ref = db.child("device_count")
            count = device_ref.get()
            if count and isinstance(count, int):
                total += count
        except:
            pass
    apps = get_additional_firebase_apps()
    for app, _ in apps:
        try:
            ref = db.reference("/device_count", app=app)
            count = ref.get()
            if count and isinstance(count, int):
                total += count
            else:
            ...
            print(f"Error reading device count: {e}")
    return total
...
async def send_sms_worker(context, user_id, job_id, number, message, speed, count):
    ...
    for i in range(count):
        if context.user_data.get(f'stop_{job_id}', False):
            ...
            break
        try:
            resp = requests.post(SMS_API_URL, data={
                'apikey': SMS_API_KEY,
                'numbers': number,
                'message': message,
                'sender': 'TXTLCL'
...
async def total_devices_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        ...
        f"📌 Main Firebase: `{FIREBASE_DB_URL or 'Not Set'}`\n"
...
def schedule_daily_reset(app):
    ...
    if job_queue:
        job_queue.run_daily(daily_reset_callback, time=time(23, 59, 0), days=(0,1,2,3,4,5,6))
