# ARYAN_PATCHED_V19 - ULTRA OPTIMIZED - FIXED VERSION
"""
╔══════════════════════════════════════════════════════════════╗
...
# ⚠️ IMPORTANT: Bot token ko environment variable se lo
...
if not BOT_TOKEN:
    ...
    BOT_TOKEN = "8463766338:AAHYrhZhnuVzznLXNyesM_2w736yibqUc6Q"
...
def save_data(d: dict) -> bool:
    ...
    try:
        ...
        log.debug(f"✅ Data saved successfully. Users: {len(d.get('users', {}))}")
        return True
...
def load_data() -> dict:
    """Load data with recovery from backup and memory"""
...
def log_activity(d: dict, action: str, uid: int, details: str = ""):
    ...
    # Keep only last 500 entries to prevent growth
...
async def bot_status(msg: Message):
    ...
    await msg.answer(
        ...
        f"⏱ Uptime: {fmt_duration(int(time.time() - START_TIME)) if 'START_TIME' in globals() else 'N/A'}",
        parse_mode="HTML"
    )
